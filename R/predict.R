...
predict.ZlmFit <- function(object,newdata = NULL, modelmatrix=NULL){
	...
	contrCovC=aaply(.data = vcov(object,"C")[colnames(modelmatrix),colnames(modelmatrix),],.margins = 3,.fun = function(x) {((modelmatrix)%*%MAST:::complexifyNA(x)%*%t(modelmatrix))})
	contrCovD=aaply(.data = vcov(object,"D")[colnames(modelmatrix),colnames(modelmatrix),],.margins = 3,.fun = function(x) {((modelmatrix)%*%MAST:::complexifyNA(x)%*%t(modelmatrix))})
	contrCovC=array(MAST:::uncomplexify(contrCovC),dimnames=list(rownames(contrCovC),colnames(contrCovC),colnames(contrCovC)),dim=c(nrow(contrCovC),ncol(contrCovC),ncol(contrCovC)))
	contrCovD=array(MAST:::uncomplexify(contrCovD),dimnames=list(rownames(contrCovD),colnames(contrCovD),colnames(contrCovD)),dim=c(nrow(contrCovD),ncol(contrCovD),ncol(contrCovD)))
