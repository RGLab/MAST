...
\begin{comment}
  ...
  Sets of single cell assays are stored in the \texttt{SCASet}
  class. A constructor for SCASet is provided to construct an SCASet
  directly from a data frame.  Alternatively, a SingleCellAssay or
  derived class can be \texttt{split} on an arbitray variable to
  produce an SCASet.
