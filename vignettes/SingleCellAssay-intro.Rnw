...
zlm.output <- zlm.SingleCellAssay(~ Population + Subject.ID, vbeta.1)
show(zlm.output)
## returns an array of both discrete and continuous
coefsAndCI <- adply( c('C', 'D'), 1, function(component){
    ...
    ci.lo <- coefs-se2
    ci.hi <- coefs+se2
    data.frame(primerid=row.names(coefs), coefs, ci.lo, ci.hi)
})

mcoefAndCI <- melt(coefsAndCI)
...
print(pvalue)
@
In fact, the \texttt{zlm} framework is quite general, and has wrappers for a variety of  modeling functions that accept \texttt{glm}-like arguments to be used, such as mixed models (using \texttt{lme4}) and Bayesian regression models (using \textt{arm}).

to test if the factor \texttt{Population} explains a significant amount of the variation by fitting models with and without \texttt{Population} as an explanatory factor.  
...
library(lme4)
lmer.output <- zlm.SingleCellAssay(~ Stim.Condition +(1|Subject.ID), vbeta.1, method='glmer',hypothesis='Stim.Condition', type='LRT', silent=TRUE)
@
