...
library(ggplot2)
library(reshape)
zlm.output <- zlm.SingleCellAssay(~ Stim.Condition + Subject.ID, vbeta.1, hypothesis='Stim.Condition', type='LRT')
dimnames(zlm.output)
...
library(lme4)
lmer.output <- zlm.SingleCellAssay(~ Stim.Condition +(1|Subject.ID), vbeta.1, method='glmer',hypothesis='Stim.Condition', type='LRT')
@
