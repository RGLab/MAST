...
```
But really we are doing an analysis of covariance (ANCOVA), where we ask if at each value of `cngeneson` if the stimulated cells differ from the unstimulated, in multiple module components.
...
To avoid setting the server farm alight, the result of such a bootstrap has been pre-calculated.
```{r boots, eval=FALSE}
#bootstrap
