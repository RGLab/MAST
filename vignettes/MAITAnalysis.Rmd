...
```{r libraries,echo=FALSE, error=FALSE,echo=TRUE,results='hide',warnin=FALSE}
...
There is also a constructor, `FromFlatDF` for flattened data where measurments, and cell and feature covariates are intermingled.
...
```
We'll limit ourselves to genes that are found in at least `r freq_expressed` fraction of the sample (since we won't have any power to conclude much about less frequent transcripts).
...
## Heatmap of MAITs based on most differentially expressed genes
 ...
 DE genes between stimulated and non-stimulated MAIT cells. 
...
To avoid heating up the server farm too much, the result of such a bootstrap has been pre-calculated.
```{r boots, eval=FALSE}
#bootstrap
