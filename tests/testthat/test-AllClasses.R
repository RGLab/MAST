...
test_that('Create a two-column id SingleCellAssay', {
  tmp <- FromFlatDF(doubleid, idvars=c('id1', 'id2'), geneid='f1', primerid='f1', measurement='et', check_logged = FALSE)
  expect_that(tmp, is_a('SingleCellAssay'))
})

intprimer <- FromFlatDF(integerprimers, idvars='id1', geneid='f1', primerid='f1', measurement='et',  check_logged = FALSE)
...
tmp <- FromFlatDF(singleid, idvars='id1', geneid='f1', primerid='f1', measurement='et',  check_logged = FALSE)
test_that('Create a one-column id SingleCellAssay', {
    expect_that(tmp, is_a('SingleCellAssay'))
})

test_that('duplicate primers doesn\'t throw error', {
  expect_warning(sc <- FromFlatDF(dataframe=duplicateprimers, idvars='id1', geneid='f1', primerid='f1', measurement='et',  check_logged = FALSE), 'A')
  expect_equal(nrow(sc), 5)
  expect_warning(sc <- FromFlatDF(dataframe=duplicateprimers2, idvars='id1', primerid='primerid', measurement='et',  check_logged = FALSE), 'A')
