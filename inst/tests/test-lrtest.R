...
test_that('LRT and zlm are equivalent', {
    ...
    zlm2 <- hushWarning(zlm.SingleCellAssay(~ Subject.ID, fd.spl[[1]], silent=FALSE), fixed('At least one component failed to converge'))
    lrout2 <- hushWarning(lrTest(zlm2, 'Subject.ID'), fixed('At least one component failed to converge'))
